...
import { Save, Plus, Trash2, ArrowLeft, Loader, Settings, RotateCcw } from 'lucide-react';
                        ...
                        <div className="divide-y divide-white/5">
                            {items.map((cp, idx) => (
                                <div key={cp.id} className="grid grid-cols-12 gap-4 p-4 items-center">
                                    ...
                                    <div className="col-span-7"><textarea value={cp.description} onChange={e => setItems(items.map(i => i.id === cp.id ? {...i, description: e.target.value} : i))} className="w-full bg-black/20 border border-white/10 rounded-lg p-2 text-white text-sm" rows="1" /></div>
                                    <div className="col-span-3"><input value={cp.method} onChange={e => setItems(items.map(i => i.id === cp.id ? {...i, method: e.target.value} : i))} className="w-full bg-black/20 border border-white/10 rounded-lg p-2 text-white text-sm" /></div>
