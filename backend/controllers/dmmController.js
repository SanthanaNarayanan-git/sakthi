...
// --- Fetch Bulk Data for PDF Export ---
exports.getBulkData = async (req, res) => {
    try {
        const { fromDate, toDate } = req.query;
        const request = new sql.Request();
        
        let query = `SELECT * FROM DmmSettingParameters`;
        if (fromDate && toDate) {
            query += ` WHERE RecordDate BETWEEN @fromDate AND @toDate`;
            request.input('fromDate', sql.Date, fromDate);
            request.input('toDate', sql.Date, toDate);
        }
        query += ` ORDER BY RecordDate ASC, Shift ASC, RowIndex ASC`;
        ...
        const customRes = await customReq.query(`
            SELECT cv.rowId, cv.columnId, cv.value 
            FROM DmmCustomColumnValues cv
            INNER JOIN DmmSettingParameters p ON cv.rowId = p.id
            ${fromDate ? 'WHERE p.RecordDate BETWEEN @fromDate AND @toDate' : ''}
        `);
